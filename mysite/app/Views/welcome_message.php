...
<head>
    ...
    <script>
        ...
        function printTime(time){
            return document.write(localTime(time));
        }
        ...
        function showAjaxData(data){
            if(data.code === 0){
                ...
                lastPage = count > 0 ? Math.ceil(count / size) : 1;
                document.getElementById('pageData').innerHTML = `当前第${page}页，每页${size}条，共${count}条数据，最后一页${lastPage}`;
                let str = ""
                data.data.users.forEach(item => {
                    str += `用户ID：${item.id}，用户名：${item.user_name}，年龄：${item.age}，性别：${item.sex}，创建时间：${item.creater_at}，更新时间：${item.update_at}<br/>`;
                });
                data = str;
            }else{
                data = '操作失败！';
        ...
        function firstData(){
            loadData('/about?page=1');
        }

        function prevData(){
            if (page <= 1) return;
            loadData(`/about?page=${page - 1}`);
        }
...
<header>
    ...
    <div class="heroe">
        <h1><script>printTime('<?= $date ?>')</script></h1>
        ...
        <p>
            <button onclick="loadData('/about')" style="padding: 10px 20px; cursor: pointer;">加载数据</button>
            <button onclick="loadData('/about/add')" style="padding: 10px 20px; cursor: pointer;">添加数据</button>
            <button onclick="loadData('/about/delete?id=1')" style="padding: 10px 20px; cursor: pointer;">删除数据</button>
            <button onclick="loadData('/about/update?id=1')" style="padding: 10px 20px; cursor: pointer;">更新数据</button>
        </p>
        
        <p>
            <button onclick="firstData()" style="padding: 10px 20px; cursor: pointer;">首页</button>
            <button onclick="prevData()" style="padding: 10px 20px; cursor: pointer;">上一页</button>
            <button onclick="nextData()" style="padding: 10px 20px; cursor: pointer;">下一页</button>
            <button onclick="lastData()" style="padding: 10px 20px; cursor: pointer;">末页</button>
        </p>

        <p id="pageData">pageData</p>

        <p id="ajaxData">ajaxData</p>
